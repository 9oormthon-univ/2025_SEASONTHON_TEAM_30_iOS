...
protocol ChallengeStatusServiceProtocol {
    ...
    func getUserTitles() async throws -> [UserTitle]
...
class ChallengeStatusService: ChallengeStatusServiceProtocol {
    ...
    func updateActiveTitle(titleId: String) async throws {
        ...
        // 반환 값이 없을 경우의 요청
        let _: UpdateActiveTitleResponse = try await APIManager.shared.request(
...
class MockChallengeStatusService: ChallengeStatusServiceProtocol {
    ...
    func getUserTitles() async throws -> [UserTitle] {
            return [
                UserTitle(titleId: "1", title: "열정왕", titleColor: "#FF0000"),
                UserTitle(titleId: "2", title: "꾸준러", titleColor: "#00FF00"),
                UserTitle(titleId: "3", title: "성실맨", titleColor: "#0000FF")
            ]
    }
